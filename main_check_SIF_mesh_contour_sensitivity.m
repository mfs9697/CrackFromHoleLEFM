...
%% ============================================================
% 2. Mesh + contour-radius sensitivity loop
%% ============================================================
...
% Relative radii
rI_over_a0_list = .5:.1:.7;

% Absolute radii not used in this mode
rI_abs_list = [];
% Debug controls.
useDebugSIF = true;      % true: call SIF_LEFM_circle2_debug through local call below
debugVerbose = false;    % set true only for one or two selected cases
debugPlot = false;       % set true only for one selected case
...
% Optional: store selected debug outputs for later inspection.
DebugStore = struct();
DebugStore.items = {};
DebugStore.count = 0;
...
for ia = 1:numel(a0_list)
    ...
    for im = 1:numel(meshScaleList)
        ...
        % Scale the mesh, then override the appendix length.
        ...
        for it = 1:numel(thetaProbe)
            ...
            try
                ...
                % ----------------------------------------------------
                % Choose contour-radius list for this case
                % ----------------------------------------------------
                ...
                for ir = 1:numel(rI_list)
                    ...
                    try
                        ...
                        % Extract selected debug diagnostics if available.
                        ...
                        fprintf(['  rI/a0 = %.4f | rI = %.4e | ', ...
                            'KI = %.8e | KII = %+ .8e | KII/KI = %+ .8e'], ...
                            rfac, rI, KI, KII, KII_over_KI);

                        if isfinite(diag_JII_over_absint)
                            fprintf(' | JII/int|J2| = %+ .3e | J2 changes = %g', ...
                                diag_JII_over_absint, diag_J2_sign_changes);
                        end
                        fprintf('\n');

                        % Store debug data only for selected cases to avoid
                        % creating a huge Results object.
                        if useDebugSIF && isstruct(AuxSIF) && isfield(AuxSIF, 'Dbg')
                            keepThisDebug = ...
                                abs(thetaDeg) < 1e-12 && ...
                        ...
                        fprintf('  rI/a0 = %.4f | rI = %.4e FAILED: %s\n', ...
                            rfac, rI, MEinner.message);
                    end
                end

            catch MEouter
                fprintf('  theta failed before radius loop: %s\n', MEouter.message);

                % Use available radius list for failed rows.
                switch lower(strtrim(radiusMode))
                    case 'relative'
                        rfac_list = rI_over_a0_list(:).';
...
fprintf('\n=== Full mesh-appendix-contour sensitivity table ===\n');
disp(T);
