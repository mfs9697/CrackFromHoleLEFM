function [KI, KII, Dbg] = SIF_LEFM_circle2_debug(mesh, U, V, mat, rI, varargin)
%SIF_LEFM_CIRCLE2_DEBUG
% Debug version of SIF_LEFM_circle2.
%
% Computes KI, KII using a circular J-integral with
% Ishikawa--Kitagawa--Okamura mode separation, and returns detailed
% diagnostic information about the contour integration.
%
% Usage:
%   [KI, KII, Dbg] = SIF_LEFM_circle2_debug(mesh, U, V, mat, rI)
%   [KI, KII, Dbg] = SIF_LEFM_circle2_debug(..., 'nthet', 200, 'plot', true)
%
% Inputs:
%   mesh   : struct with fields
%              .coord    [nT6 x 2]  T6 node coordinates
%              .connect  [nElem x 6] T6 connectivity
%              .coord3   [nT3 x 2]  T3 parent coordinates
%              .connect3 [nElem x 3] T3 parent connectivity
%   U      : displacement vector [ux1; uy1; ux2; uy2; ...]
%   V      : crack polyline, last point is crack tip
%   mat    : struct with .E, .nu, .Dmat
%   rI     : circular contour radius. If empty, uses 0.7*crack length.
%
% Name-value options:
%   'nthet'      : number of contour sampling points, default 100
%   'eps_th'     : angular exclusion near crack faces, default 1e-3
%   'plot'       : true/false diagnostic plots, default false
%   'verbose'    : true/false console diagnostics, default false
%
% Outputs:
...
%   Dbg     : diagnostic struct
    ...
    elseif isfield(mat, 'D') && ~isempty(mat.D)
        ...
        error('SIF_LEFM_circle2_debug:MissingDmat', ...
    ...
    if size(V,1) < 2 || size(V,2) ~= 2
        error('SIF_LEFM_circle2_debug:BadCrackPolyline', ...
    ...
    if ~(isnumeric(rI) && isscalar(rI) && isfinite(rI) && rI > 0)
        error('SIF_LEFM_circle2_debug:BadRadius', ...
    ...
    if Le <= eps
        error('SIF_LEFM_circle2_debug:DegenerateLastSegment', ...
    ...
    %% ------------------------------------------------------------
    if eps_th >= pi/2
        error('SIF_LEFM_circle2_debug:BadEpsTheta', ...
    ...
    if any(outsideP) || any(outsideQ)
        ...
        error('SIF_LEFM_circle2_debug:ContourOutsideMesh', ...
    ...
    for k = 1:nthet
        ...
        % ---------------------------------------------------------
        % Mode separation: Ishikawa--Kitagawa--Okamura
        % ---------------------------------------------------------
        epsI = 0.5 * [ ...
            epsP(1) + epsQ(1);
            epsP(2) + epsQ(2);
        ...
        duI = 0.5 * [ ...
            du_dx1_P(1) + du_dx1_Q(1);
            du_dx1_P(2) - du_dx1_Q(2) ];
...
function print_debug_summary(Dbg)
    ...
    fprintf('\nSIF_LEFM_circle2_debug summary:\n');
    fprintf('  rI                 = %.8e\n', Dbg.rI);
    fprintf('  rI/a_tot           = %.8e\n', Dbg.rI_over_a_tot);
    fprintf('  KI                 = %.8e\n', Dbg.KI);
    fprintf('  KII                = %.8e\n', Dbg.KII);
...
function plot_debug_contour(Dbg, coord3, connect3, V)
    ...
    figure('Name', 'SIF debug: contour points', 'Color', 'w');
    clf;
    hold on;
    axis equal;
    box on;
    ...
    xlabel('x');
    ylabel('y');
    title(sprintf('Circular contour, r_I = %.4e', Dbg.rI));
...
function plot_debug_integrands(Dbg)
    ...
    figure('Name', 'SIF debug: J integrands', 'Color', 'w');
    clf;
    ...
    title(sprintf('Mode-I separated integrand, J_I = %.4e', Dbg.JI));
    ...
    title(sprintf('Mode-II separated integrand, J_{II} = %.4e', Dbg.JII));
end
...
function plot_debug_elements(Dbg)
    ...
    figure('Name', 'SIF debug: element and barycentric diagnostics', ...
        'Color', 'w');
    clf;
...
function must_have(S, field)
    if ~isstruct(S) || ~isfield(S, field) || isempty(S.(field))
        error('SIF_LEFM_circle2_debug:MissingField', ...
